...
fn extract_hash(init_data: &str) -> Result<(&str, &str), ValidationError> {
    ...
    let (base_data, hash) = init_data.split_once("&hash=")
        .ok_or_else(|| ValidationError::MissingField("hash field not found".into()))?;
    ...
    if hash.len() != 64 {
        return Err(ValidationError::InvalidHashFormat(
            format!("hash length is {}, expected 64", hash.len())
        ));
    }
    ...
    let validate_fn = Closure::wrap(Box::new(move |init_data: String| -> bool {
        let result = (|| -> Result<bool, ValidationError> {
            ...
            for decoded_pair in pairs {
                let (key, _) = decoded_pair.split_once('=')
                    .ok_or_else(|| ValidationError::InvalidQueryString("malformed query pair".into()))?;
